...
const randomAutoMessageTimers = new Map<
  string,
  ...
  try {
    ...
    whatsappClient.initialize().catch((err) => {
      ...
      mainWindow?.webContents.send(
        IPCChannels.WHATSAPP_AUTH_FAILURE,
        err.message,
      );
    });
  } catch (error) {
...
async function loadChats() {
  ...
  try {
    ...
    for (const chat of wwebChats) {
      ...
      const savedMessages = conversationStore.get(
...
async function handleIncomingMessage(message: Message) {
  try {
    ...
    if (deleteMatch) {
      ...
      console.log(
        `[/delete] Removed last ${actuallyDeleted} messages from chat ${chatId}`,
      );
      return;
    }
    ...
    if (timestampMatch) {
      ...
      await sendNotifMessage(
        chatId,
        `Timestamps are now ${enable ? "ON" : "OFF"}`,
      );
    ...
    // /model {modelName} command — change the LLM model
    const modelMatch = body.match(/^\/model\s+(\S+)$/i);
    if (modelMatch) {
      const chatId = message.from;
      const newModel = modelMatch[1];

      // Read current LLM settings, update the model, persist
      const llmStore = new Store({ name: "llm-settings" });
      llmStore.store = { ...llmStore.store, model: newModel };
      ...
      // Ephemeral notification: send but immediately remove from history
      await sendNotifMessage(chatId, `Model changed to: ${newModel}`);
    ...
    const shouldAutoReply =
      (autoReplyChatIds.has(message.from) ||
        settingsStore.store.autoReplyToAll) &&
      !message.fromMe;
...
async function applyReplyDelay(chatId: string) {
  ...
  try {
    ...
    if (wwebChat) {
      ...
      console.log(
        `[typing] Showing typing state for ${delayMs}ms in chat ${chatId}`,
      );
      ...
      console.warn(
        `[typing] Could not find chat ${chatId}, falling back to plain delay`,
      );
      await new Promise((resolve) => setTimeout(resolve, delayMs));
    }
  } catch (error) {
...
function scheduleRandomAutoMessage(chatId: string) {
  ...
  console.log(
    `[random-auto] Scheduling message for chat ${chatId} in ${Math.round(delayMs / 60000)} min`,
  );
      ...
      for (const segment of segments) {
        ...
        if (resolution.type === "image") {
          if (!resolution.media) {
            ...
            console.warn(
              `[whatsapp] Skipping missing image: ${resolution.filename}`,
            );
            continue;
          }
  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (resolution.type === "image") {
        ...
        await applyReplyDelay(chatId);
        console.log(
          `Sending image ${i + 1}/${segments.length}: ${resolution.filename}`,
        );
...
async function logoutWhatsApp(): Promise<{
  success: boolean;
  message: string;
}> {
  if (!whatsappClient) {
  ...
  try {
    ...
    const authPath = path.join(
      process.cwd(),
      ".wwebjs_auth",
      "session-whatsapp-llm-assistant",
    );
...
function setupIPCHandlers() {
  ...
  ipcMain.on(
    IPCChannels.TOGGLE_AUTO_REPLY,
    (_, chatId: string, enabled: boolean) => {
      if (enabled) {
  ...
  // Send message
  ipcMain.handle(
    IPCChannels.SEND_MESSAGE,
    async (_, chatId: string, text: string) => {
