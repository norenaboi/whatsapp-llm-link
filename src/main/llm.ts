...
function prepareMessagesForLLM(chat: Chat): LLMMessage[] {
  ...
  const messages: LLMMessage[] = [
    {
      role: "system",
      content: llmSettings?.systemPrompt + imageInstructions,
    },
  ];

  // Add recent messages from chat
  const chatMessages = [...chat.messages].slice(
    -(llmSettings?.maxHistoryLength || 0) * 2,
  );
...
async function generateCustomEndpointResponse(
  messages: LLMMessage[],
): Promise<string | null> {
  ...
  const response = await axios.post(llmSettings.apiEndpoint, payload, {
    headers: {
      ...
      ...(llmSettings.customApiKey && {
        Authorization: `Bearer ${llmSettings.customApiKey}`,
      }),
    },
  });
  ...
  const ollamaContent =
    response.data?.message?.content ?? response.data?.response; // /api/chat // /api/generate
...
export async function testLLMConfiguration(
  ...
  const originalSettings: LLMSettings | undefined = llmSettings;
  ...
  try {
    ...
    // Restore original settings only if they existed before the test
    if (originalSettings) {
